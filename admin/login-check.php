...
if (!ctype_alnum($username) OR !ctype_alnum($password)) {
	...
	// jika data tidak ada, alihkan ke halaman login dan tampilkan pesan = 1
		...
		$query = mysqli_query($mysqli, "SELECT * FROM tbl_konselor WHERE username='$username' AND password='$password'")
		...
		if ($rows > 0) {
			...
			$_SESSION['id_user']   = $data['id_konselor'];
			$_SESSION['username']  = $data['username'];
			$_SESSION['password']  = $data['password'];
			$_SESSION['nama_user'] = $data['nama_konselor'];
			$_SESSION['hak_akses'] = 'konselor';
